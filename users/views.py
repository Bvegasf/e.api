...
from users.API.serializer import UserTokenserializador
...
class UserToken(APIView):
    def get(self, request, *args, **kwargs):
        ...
        try:
            user_token = Token.objects.get(user =UserTokenserializador().Meta.model.objects.filter(username = username).first() )
...
class Login(ObtainAuthToken):
    ...
    def post(self,request,*args,**kwargs):
      # se le envia al serializador el usuario y el password
        login_serializer = self.serializer_class(data = request.data, context = {'request':request})
       
        if login_serializer.is_valid():
           #login_serializdor retorna el usuario validado
            user = login_serializer.validated_data['user']
            if user.is_active:
                token,created = Token.objects.get_or_create(user = user)
                user_serializer = UserTokenserializador(user)
                if created:
                    return Response({
                        'token': token.key,
...
class Logout(APIView):
    def get(self,request,*args,**kwargs):
        ...
        try:
            ...
            return Response({'error':'No existe un usuario con los datos indicados'},
                                            status=status.HTTP_400_BAD_REQUEST)
